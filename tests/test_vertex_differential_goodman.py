...
def test_vertex_differential_goodman():
    ...
    for k in [3, 4, 5, 6, 7]:
        ...
        # Exact integer dot product: nonzero iff the differential is nonzero
        res_int = density_nums @ grad_num
        if not np.all(res_int == 0):
            bad = np.where(res_int != 0)[0]
            failures.append(f"W_{k}: nonzero at flag indices {bad.tolist()}, values {res_int[bad].tolist()}")

    assert not failures, "\n".join(failures)


if __name__ == "__main__":
    test_vertex_differential_goodman()
    print("Test passed: The vertex differential for Goodman expression is exactly zero at W_k for k=3,4,5,6,7.")
