...
def test_edge_differential_fisher():
    ...
    for n in range(11):
        ...
        if not all(r >= 0 for r in res_int):
            ...
            failures.append(f"S={S} (x={x}): negative at {bad}")

    assert not failures, "\n".join(failures)


if __name__ == "__main__":
    test_edge_differential_fisher()
    print("All tests passed for Razborov's edge differential (exact rational mode).")
