...
def test_edge_differential_c5():
    ...
    # Exact integer dot product: shape [len_flags_E]
    res_int = np.array(
    ...
    if not np.all(res_int >= 0):
        ...
        failures.append(f"edge differential negative at flag indices {bad.tolist()}, values {res_int[bad].tolist()}")

    assert not failures, "\n".join(failures)


if __name__ == "__main__":
    test_edge_differential_c5()
    print("Test passed: The edge differential is non-negative for C5 inducibility.")
